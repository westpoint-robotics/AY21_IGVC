...
bool PublishControl::teleop_on = false; // unique to teleop
...
PublishControl::PublishControl()
{
  ...
  state_sub = n.subscribe("/gem/operation_mode", 20, &PublishControl::callback_state);  // unique to teleop
...
/*
 * Called when a state message is received  // unique to teleop
 */
void PublishControl::callback_state(const std_msgs::Int8::ConstPtr& msg){
    if (msg->data == 1){
        if (!teleop_on) {
            teleop_on = true;
        }
...
void PublishControl::callback_control(const sensor_msgs::Joy::ConstPtr& msg)
{
  try
  {
    // Only send messages when enabled, or when the teleop_on variable changes between enabled/disabled
    check_is_enabled(msg);

    if ((local_enable == true || local_enable != prev_enable)&& teleop_on)
    {
      // Steering
