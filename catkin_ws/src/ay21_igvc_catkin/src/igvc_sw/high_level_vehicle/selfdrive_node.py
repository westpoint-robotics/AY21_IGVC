...
run_modes = {'\n':[0,"Regular Self-drive"],'q1':[1,'E-Stop Qualification'],'q2':[2,'Straight Lane Keeping Qualification'],\
            'q3':[3,'Left Turn Qualification'],'q4':[4,'Right Turn Qualification'], 'q5':[5,'Stop Sign Detection Qualification'], 'q6':[6,'Parallel Parking Qualification']}
...
def show_options():
    print  "\r\t\tOPTIONS:\
            ...
            \n\r\t\t * Parallel Parking Qualification    'q6'"
    return
...
def countdown_to_run(mode_nm):
    ...
    while n > -1:
        if not run_status == 2:
            ...
            tm = rospy.get_time()
            if not n:
                if shift_state == 3 or shift_state == 1:
                    print "\r\t-- RUNNING!"
                    brake_cmd.f64_cmd = 0
